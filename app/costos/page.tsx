...
function CatalogoCOGS({ country, brand, search }: { country: CountryKey; brand: string; search: string }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    r = r.filter(p => !/ebook|eook|guía|guia|brocha|protocolo|recetario|protección|proteccion|calendario|hábitos|habitos|menú|menu|plan de gym|plan anti|método|metodo|ritual|set |kit |collar|agenda|21d|reto |challenge|gratis|upsell|vitamina c|youtful|reafirmante|rendimiento extendido|rendimiento m[aá]ximo|lifting desde|pureza extendida|poros|glow desde|fórmula pro|formula pro|rutina anti|tracker/i.test(p.title));
    return r;
